...
class BookService {
    ...
    public async updateBook(book: IBookModel) {
        const error = book.validateSync(); // If no error - returns null.
        if (error) throw new ValidationError(error.message);
        const updatedBook = await BookModel.findByIdAndUpdate(book._id, book, { returnOriginal: false }).exec(); // returnOriginal: false --> Return the updated book and not the original one
    ...
    public getSomeBooks() {
        ...
        return BookModel.find(
            ...
            ["-_id", "name", "price"]) // Projection = SELECT
            .exec();
    }
